...
Single-file app (`SpeechToText/main.swift`, ~2700 lines):
...
│   ├── SpeechyIconView (gradient app icon with flag overlay)
│   └── WaveformView (11 gradient bars visualizing real-time audio RMS)
...
├── SettingsView (sidebar with 3 tabs)
│   ├── SettingsTab (hotkey configuration)
│   ├── AdvancedTab (model, delay, audio device, general)
...
- Per-slot language selection from 29 supported languages
...
### Overlay & Waveform
- Recording state: gradient Speechy icon with flag overlay + 11 animated waveform bars with gradient coloring and glow effect
- Processing state: spinner animation
- Window floats at bottom center of screen (120x150px, rounded dark background)

### Transcription Flow
1. Hotkey detected → overlay shows Speechy icon with flag
...
| `SpeechToText/Tests/DesktopTests.swift` | Test suite (58 tests) |
...
Compiles with `-DTESTING` flag and runs 58 tests covering: HotkeyConfig, ModelType, TranscriptionEntry, AudioInputDevice, supported languages, filterNonSpeech, getFlag, matchesConfig, and SettingsManager history.
